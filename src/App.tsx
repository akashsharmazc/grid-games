...
import type { TTTScore, PersistedSnapshot, TTTCell } from './types';
...
export default function App() {
  ...
  const tttResult = useMemo(() => 
    computeTTTWinner(gameState.grid as TTTCell[], gameState.n), 
  ...
  const handleCellActivate = useCallback((index: number) => {
    ...
    if (gameState.mode === "tictactoe") {
      const grid = gameState.grid as TTTCell[];
    ...
    seq.forEach((cellIdx, i) => {
      setTimeout(() => {
        ...
        gameState.updateGrid(() => {
          const newGrid = Array(gameState.n * gameState.n).fill(false) as boolean[];
